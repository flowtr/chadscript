...
pub fn run(source_path: &Path) -> Result<(), Box<dyn Error>> {
					...
					if left_expr.is_lit() && right_expr.is_lit() {
						...
						let result = match binary_op {
							swc_ecma_ast::BinaryOp::Add => {
								builder.build_int_add(x, y, "main")
							}
							swc_ecma_ast::BinaryOp::Sub => {
								builder.build_int_sub(x, y, "main")
							}
							swc_ecma_ast::BinaryOp::Div => {
								builder.build_int_signed_div(x, y, "main")
							}
							swc_ecma_ast::BinaryOp::Mul => {
								builder.build_int_mul(x, y, "main")
							}
							_ => i64_type.const_int(0u64, true),
						};
						builder.build_return(Some(&result));
						let e = module.create_jit_execution_engine(
							OptimizationLevel::None,
						)?;
						unsafe {
							let r = e
								.get_function::<unsafe extern "C" fn() -> u64>(
									"main",
								)?
								.call();
							println!("{:?}", r);
